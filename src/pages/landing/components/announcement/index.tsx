...
import { animationKeyframes } from "./animation";
...
const Announcement: React.FunctionComponent<ComponentProps> = ({
  ...
  const expirationDate = React.useMemo(() => {
    ...
    date.setDate(today.getDate() + (daysToLive !== undefined ? daysToLive : 5));
    return date;
  }, [daysToLive]);

  const secondsBeforeBanner = React.useMemo(() => {
    return secondsBeforeBannerShows !== undefined
