...
const animationDuration = 1; // Change animation duration here
const variants = {
...
const SplashScreen: React.FC = () => {
  const { setShouldPlayAudio } = useAnimation();

  useEffect(() => {
    // Don't play audio during splash screen
    setShouldPlayAudio(false);
    
    return () => {
      // No need to handle anything on unmount now that SunLoop is removed
    };
  }, [setShouldPlayAudio]);

  return (
    <section className="back">
      <animated.div className={`preload`}>
        <div className={styles.splash_screen__section}>
          <div>
            ...
            >
              ...
              <motion.path
                fill="#8b4513" // Update to match paper theme
                stroke="#8b4513" // Update to match paper theme
                strokeWidth="0.5"
                variants={variants}
                initial="initial"
                animate="animate"
